...
from app.core.config import ENABLE_LIVE_MARKET_DATA_FETCH
...
class LiveDividendYieldProvider:
    """Loads forward dividend estimates from an external market-data source."""
    ...
    def _fetch_estimate(self, ticker: str) -> DividendYieldEstimate:
        try:
            ...
            return self._zero_estimate(ticker, frequency_label="unknown")
    ...
    def _zero_estimate(self, ticker: str, *, frequency_label: str) -> DividendYieldEstimate:
        return DividendYieldEstimate(
            ticker=ticker,
            annualized_dividend=0.0,
            annual_yield=0.0,
...
class DividendYieldService:
    """Resolves dividend overlays for expected-return calculations.

    Stored estimates generated by the admin refresh flow are the primary source.
    Live fetch is an explicit fallback path and remains disabled by default.
    """
    ...
    def __init__(
        ...
        *,
        allow_live_fallback: bool = ENABLE_LIVE_MARKET_DATA_FETCH,
        live_provider: LiveDividendYieldProvider | None = None,
    ) -> None:
        self.repository = repository or ManagedUniverseRepository()
        self.allow_live_fallback = allow_live_fallback
        self.live_provider = live_provider or LiveDividendYieldProvider()
        self._cache: dict[str, DividendYieldEstimate] = {}
    ...
    def estimate_for_ticker(self, ticker: str) -> DividendYieldEstimate:
        ...
        estimate = self._load_stored_estimate(normalized)
        if estimate is None:
            estimate = self._fallback_estimate(normalized)
        self._cache[normalized] = estimate
        return estimate

    def _load_stored_estimate(self, ticker: str) -> DividendYieldEstimate | None:
        if not self.repository.is_configured():
            return None
        try:
            return self.repository.get_dividend_yield_estimate(ticker)
        except Exception:
            return None

    def _fallback_estimate(self, ticker: str) -> DividendYieldEstimate:
        if not self.allow_live_fallback:
            return DividendYieldEstimate(
                ...
                annual_yield=0.0,
                payments_per_year=0,
                frequency_label="disabled",
                last_payment_date=None,
                source="fallback",
            )
        return self.live_provider.fetch_estimate(ticker)
