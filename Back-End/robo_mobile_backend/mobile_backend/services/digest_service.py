...
class DigestService:
    ...
    # ---------------------------------------------------------------------
    # [QA] Temporary helper for manual verification on the iOS simulator.
    # Remove this method (and the route that calls it) before launch.
    # ---------------------------------------------------------------------
    def synthesize_for_qa(self, account_id: int, mode: str) -> dict:
        """Cache a synthetic digest in a requested state. QA only."""
        end_date = datetime.now(timezone.utc).date()
        start_date = end_date - timedelta(days=DIGEST_PERIOD_DAYS)
        now_utc = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        if mode == "unavailable":
            ...
            digest = {
                **common,
                ...
                "available": True,
                "narrative_ko": (
                    "[QA] 이번 주 포트폴리오는 +6.0%의 강한 수익을 기록했습니다. "
                    "DBC(금)가 420,000원, NFRA(인프라 채권)가 180,000원 상승하며 "
                    "긍정적인 영향을 주었습니다. 이는 정상적인 변동 범위 내에 있습니다."
                ),
                "has_narrative": True,
                "drivers": [
                    {
                        ...
                        "weight_pct": 8.4,
                        "return_pct": 25.0,
                        "contribution_won": 420_000,
                        "explanation_ko": "[QA] DBC(금)는 인플레이션 헤지 수단으로서 주목받으며 강한 상승세를 보였습니다.",
                    },
                    {
                        "ticker": "NFRA",
                        "name_ko": "인프라 채권",
                        ...
                        "weight_pct": 7.1,
                        "return_pct": 12.0,
                        "contribution_won": 180_000,
                        "explanation_ko": "[QA] NFRA(인프라 채권)는 안정적인 인프라 수요와 함께 상승했습니다.",
                    },
                ],
                "detractors": [],
                "sources_used": ["QA mock"],
                "benchmark_7asset_return_pct": 1.0,
                "benchmark_bond_return_pct": 0.1,
            }
        elif mode == "negative":
            digest = {
                **common,
                ...
                "available": True,
                "narrative_ko": (
                    "[QA] 이번 주 포트폴리오는 -6.0%의 조정을 보였습니다. "
                    "LCID(신성장주)가 300,000원, NTLA(신성장주)가 300,000원 하락했습니다. "
                    "이는 시장의 정상적인 변동 범위 내에 있습니다."
                ),
                "has_narrative": True,
                "drivers": [],
                "detractors": [
                    {
                        ...
                        "weight_pct": 0.1,
                        "return_pct": -30.0,
                        "contribution_won": -300_000,
                        "explanation_ko": "[QA] LCID(신성장주)는 시장 전반의 흐름에 따라 하락했습니다.",
                    },
                    {
                        "ticker": "NTLA",
                        "name_ko": "신성장주",
                        ...
                        "return_pct": -28.0,
                        "contribution_won": -300_000,
                        "explanation_ko": "[QA] NTLA(신성장주)는 개별 기업 소식에 따라 하락했습니다.",
                    },
                ],
                "sources_used": ["QA mock"],
                "benchmark_7asset_return_pct": 1.0,
            ...
            raise ValueError(
                f"Unknown QA mode: {mode!r}. Use 'positive', 'negative', or 'unavailable'."
            )

        self.digest_repo.cache(account_id, digest)
        logger.info(
            "digest.qa.synthesize account_id=%s mode=%s", account_id, mode
        )
        return digest
